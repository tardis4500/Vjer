...
# ----------------------------------------------------------------------------
# flake8: noqa
# pylint: disable=redundant-u-string-prefix,too-many-lines,invalid-name,consider-using-f-string
...
options:
  ...
  node_count:
    description:
    - The node count for the pool. You must ensure that your Compute Engine
...
_RESOURCE_CHECKS = {'initialNodeCount': {'default': 0, 'parameters': {}},
                    'locations': {'default': [], 'parameters': {}},
                    'autoscaling': {'default': {},
                                    'parameters': {'enabled': False, 'minNodeCount': None, 'maxNodeCount': None, 'totalMinNodeCount': None, 'totalMaxNodeCount': None}},
                    'management': {'default': {}, 'parameters': {'autoRepair': False, 'autoUpgrade': False}}}
...
def main():
    ...
    module = GcpModule(
        argument_spec=dict(
            ...
            node_locations=dict(type='list', elements='str'),
            node_count=dict(required=True, type='int'),
            version=dict(type='str'),
            autoscaling=dict(
                type='dict', options=dict(enabled=dict(type='bool'), min_node_count=dict(type='int'), max_node_count=dict(type='int'),
                                          total_min_node_count=dict(type='int'), total_max_node_count=dict(type='int'))),
            management=dict(
            ...
            upgrade_settings=dict(
                type='dict', options=dict(max_surge=dict(type='int'), max_unavailable=dict(type='int'))
            ),
    ...
    # module.warn(f'******initial fetch*****\n{fetch}\n********')
    changed = False

    if fetch:
        if state == 'present':
            differences = is_different(module, fetch)
            if differences:
                update(module, differences, self_link(module))
                fetch = fetch_resource(module, self_link(module))
                changed = True
        else:
...
def create(module, link):
    ...
    return wait_for_operation(module, auth.post(link, encode_create_request(resource_to_request(module))))


def update(module, differences, link):
    auth = GcpSession(module, 'container')

    # Not all items can be updated in a single update request. There are four different update endpoints.
    updated = False
    #   setAutoscaling
    if 'autoscaling' in differences:
        updated = wait_for_operation(module, auth.post(f'{link}:setAutoscaling', {'autoscaling': differences['autoscaling']}))
        differences = is_different(module, updated)

    #   setManagement
    if 'management' in differences:
        updated = wait_for_operation(module, auth.post(f'{link}:setManagement', {'management': differences['management']}))
        differences = is_different(module, updated)

    #   setSize
    if 'initialNodeCount' in differences:
        updated = wait_for_operation(module, auth.post(f'{link}:setSize', {'nodeCount': differences['initialNodeCount']}))
        differences = is_different(module, updated)

    #   update
    if 'updates' in differences:
        updated = wait_for_operation(module, auth.put(link, differences['updates']))
...
def resource_to_request(module):
    request = {
        ...
        u'initialNodeCount': module.params.get('node_count'),
        u'locations': module.params.get('node_locations'),
...
def fetch_resource(module, link, allow_not_found=True):
    auth = GcpSession(module, 'container')
    resource = return_if_object(module, auth.get(link), allow_not_found)
    if not resource:
        return resource
    for (parameter_class, parameter_info) in _RESOURCE_CHECKS.items():
        if (parameter_class not in resource) or not resource[parameter_class]:
            resource[parameter_class] = parameter_info['default']
        for (param, default) in parameter_info['parameters'].items():
            if param not in resource[parameter_class]:
                resource[parameter_class][param] = default
    return resource
...
def is_different(module, response):
    ...
    current = response_to_hash(module, response)
    # module.warn(f'******requested*****\n{request}\n********current*****\n{current}')

    differences = {}
    for (difference_type, difference_checks) in _RESOURCE_CHECKS.items():
        if difference_type not in request:
            continue

        if not isinstance(difference_checks['default'], dict):
            if current[difference_type] != request[difference_type]:
                differences[difference_type] = request[difference_type]
            continue

        for (difference_check, default) in difference_checks['parameters'].items():
            current_value = current[difference_type][difference_check] if (difference_check in current[difference_type]) else default
            request_value = request[difference_type][difference_check] if (difference_check in request[difference_type]) else default
            if current_value != request_value:
                differences[difference_type] = request[difference_type]
                break

    if 'locations' in differences:
        differences['updates'] = {'locations': differences['locations']}
        del differences['locations']
...
def response_to_hash(module, response):
    return {
        ...
        u'locations': response.get('locations'),
        u'initialNodeCount': response.get('initialNodeCount'),
...
def encode_create_request(resource_request):
    ...
    def to_request(self):
        return remove_nones_from_dict(
            {u'enabled': self.request.get('enabled'), u'minNodeCount': self.request.get('min_node_count'), u'maxNodeCount': self.request.get('max_node_count'),
             u'totalMinNodeCount': self.request.get('total_min_node_count'), u'totalMaxNodeCount': self.request.get('total_max_node_count')}
        )
    ...
    def from_response(self):
        return remove_nones_from_dict(
            {u'enabled': self.request.get(u'enabled'), u'minNodeCount': self.request.get(u'minNodeCount'), u'maxNodeCount': self.request.get(u'maxNodeCount'),
             u'totalMinNodeCount': self.request.get(u'totalMinNodeCount'), u'totalMaxNodeCount': self.request.get(u'totalMaxNodeCount')}
        )
...
# cSpell:ignore vm googlecloudplatform suboptions preemptible autoscaler stockout machineaccount serviceaccount accesstoken jsonarg nodepool maxpodsconstraint
# cSpell:ignore shieldedinstanceconfig workloadmetaconfig upgradeoptions deepdiff
