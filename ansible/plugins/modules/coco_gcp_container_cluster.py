...
options:
  ...
  node_count:
    ...
    - This field has been deprecated. Please use nodePool.node_count instead.
    required: false
    type: int
  node_config:
    description:
...
_RESOURCE_CHECKS = {'resourceLabels': {'default': None, 'parameters': {}}}
_KEY_IGNORES = ['initialClusterVersion']
_KEY_XFORMS = {'initialClusterVersion': 'currentMasterVersion'}
...
def update(module, differences, link):
    ...
    # Resource labels cannot be set with a standard update.
    updated = False
    if 'resourceLabels' in differences:
        updated = wait_for_operation(module, auth.post(f'{link}:setResourceLabels',
                                                       {'resourceLabels': differences['resourceLabels'], 'labelFingerprint': differences['labelFingerprint']}))
        differences = is_different(module, updated)
    if 'updates' in differences:
        updated = wait_for_operation(module, auth.put(link, {'update': differences['updates']}))
...
def resource_to_request(module):
    request = {
        ...
        u'initialClusterVersion': module.params.get('cluster_version'),
...
def is_different(module, response):
    ...
    if 'resourceLabels' in differences:
        differences['labelFingerprint'] = current['labelFingerprint']

    for (key, value) in request.items():
        if key in _RESOURCE_CHECKS:
            continue
        request_key = _KEY_XFORMS.get(key, key)
        if (request_key not in current) or (value != current[request_key]):
            use_key = request_key.replace('current', '')
            differences['updates'] = {'desired' + use_key[0].upper() + use_key[1:]: value}

    # module.warn(str(differences))
    return differences
...
class Kubectl(object):
    ...
    def write_file(self):
        ...
        """
        with open(self.module.params['kubectl_path'], 'w', encoding='utf-8') as f:
...
# cSpell:ignore accesstoken jsonarg serviceaccount preemptible abac snat masterauth databaseencryption privateclusterconfig addonsconfig legacyabac networkpolicy
# cSpell:ignore defaultmaxpodsconstraint ipallocationpolicy masterauthorizednetworksconfig binaryauthorization releasechannel shieldednodes networkconfig nodeconfig
# cSpell:ignore shieldedinstanceconfig clientcertificateconfig datapath cidrblocks nodepools kubelets suboptions googleapis heapster subnetwork subnetworks googlecloudplatform
# cSpell:ignore machineaccount httploadbalancing horizontalpodautoscaling networkpolicyconfig xforms
